...
to $\mathbb{R}^{N}$, a vector $\textbf{x}$ is an \_eigenvector\_
if we have $\textbf{M}\textbf{x} = \lambda \textbf{x}$, where $\lambda$
is the associated \_eigenvalue\_. This definition is good enough for finite
dimensional vector spaces. Otherwise, we can generalize the definition of
eigenvalue to that of the \_spectrum\_ of an operator, which is the collection
...
an \_eigenbasis\_ - a set of linearly independent eigenvectors which span our space.
...
In many linear or near-linear dynamical systems, understanding the \_eigenmodes\_
...
This is known as the \_Jordan canonical form\_, and the blocks look like
...
The non-diagonalizability is related to \_nilpotent\_ elements - submatrices which
...
invariant subspace $V\_{k}$.
That is, on $V\_{k}$ it is the identity;
...
This linear map outputs onto the orthogonal subspace $(V\_{k})\_{\perp}$.
Therefore, restricted to $(V\_{k})\_{\perp}$ it is a map from that subspace
onto itself. It has some eigenvector $v\_{k+1}$. While $v\_{k+1}$ is an eigenvector
of $\textbf{M}\_{k+1}$, it is not an eigenvector of $\textbf{M}$; however,
it has no other projection onto $(V\_{k})\_{\perp}$. Therefore,
$\textbf{M}$ maps $V\_{k}\oplus\{v\_{k+1}\}$ onto itself; we now have
...
where $\textbf{T}$ is upper triangular, and $\textbf{U}$ is a \_unitary\_ matrix
...
Related to this decomposition is the \_QR decomposition\_ where $\textbf{M}$
...
\_lengths\_ invariant!

The most commonly used type of unitary transformation of a matrix is
the \_singular value decomposition\_ (SVD). We decompose our matrix
$\textbf{M}$ as
...
The columns of $\textbf{V}$ are the \_right singular vectors\_
and the columns of $\textbf{U}$ are the \_left singular vectors\_.
...
$$
\lambda\_{u}\textbf{u}\_{c} = \textbf{M}\textbf{v}\_{c},~\lambda\_{v}\textbf{v}\_{c} = \textbf{M}^{\dagger}\textbf{u}\_{c}
$$
...
$\textbf{M}^{\dagger}\textbf{M}$. These are both \_Hermitian\_ matrices
...
matrix is \_normal\_ - that is, it has an orthogonal eigenbasis. In this case
...
$$
(\textbf{M}\textbf{M}^{\dagger})\_{ij} = \sum\_{\alpha}(\textbf{v}\_{\alpha})\_{i}
(\textbf{v}\_{\alpha})\_{j}
$$
...
technique \_PCA\_. The eigenvectors $\textbf{U}$ are called \_principal components\_
...
arbitrary \_linear\_ transforms! Non-linear transforms will be even less related.
...
large $\lambda\_{i}$. Let $\textbf{w}' = \textbf{B}^{-1}\textbf{w}$; then we have
...
$$
\textbf{w}^{\dagger}\textbf{x} = \sum\_{i}(\textbf{w}')\_{i}c\_{i}e^{\lambda\_{i}t}
$$
